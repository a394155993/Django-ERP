...
class Inventory(generic.BO):
    ...
    warehouse = models.ForeignKey(Warehouse, verbose_name=_("warehouse"), on_delete=True)
    material = models.ForeignKey(Material, verbose_name=_("material"), on_delete=True)
    measure = models.ForeignKey(Measure, verbose_name=_("measure"), on_delete=True)
...
class InitialInventory(generic.BO):
    ...
    attach = models.FileField(_('attach'), blank=True, null=True, upload_to='inventory',
                              help_text=u'参考FD0002模板文档')
                ...
                for code, name, description, unit_code, unit_name, warehouse_code, warehouse_name, price, cnt in reader:
                    ...
                    try:
                        ...
                        house = Warehouse.objects.create(code=warehouse_code,
                                                         name=force_str(warehouse_name.decode('gbk')))

                    try:
                        material = Material.objects.get(code=code)
                    except Exception as e:
                        material = Material(code=code, name=force_str(name.decode('gbk')),
                                            spec=force_str(description.decode('gbk')), warehouse=house)
...
class StockIn(generic.BO):
    ...
    po = models.ForeignKey(PurchaseOrder, verbose_name=_("purchase order"), null=True,
                           limit_choices_to={"entry_status": "0"}, blank=True, on_delete=True)
...
class StockOut(generic.BO):
    ...
    org = models.ForeignKey(Organization, verbose_name=_("organization"), blank=True, null=True, on_delete=True)
    title = models.CharField(_("title"), max_length=const.DB_CHAR_NAME_40)
    project = models.ForeignKey(Project, verbose_name=_("project"), blank=True, null=True, on_delete=True)
    wo = models.ForeignKey(WorkOrder, verbose_name=_("work order"), blank=True, null=True, on_delete=True)
    description = models.TextField(_("description"), blank=True, null=True)
    amount = models.DecimalField(_("money of amount"), max_digits=14, decimal_places=4, blank=True, null=True)
    user = models.ForeignKey(User, verbose_name=_("out user"), blank=True, null=True, on_delete=True)
    status = models.CharField(_("status"), max_length=const.DB_CHAR_CODE_2, default='0', choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"), blank=True, null=True)
...
class WareReturn(generic.BO):
    ...
    org = models.ForeignKey(Organization, verbose_name=_("organization"), blank=True, null=True, on_delete=True)
    title = models.CharField(_("title"), max_length=const.DB_CHAR_NAME_40)
    out = models.ForeignKey(StockOut, verbose_name=_('StockOut'), on_delete=True)
    amount = models.DecimalField(_("money of amount"), max_digits=14, decimal_places=4, blank=True, null=True)
    user = models.ForeignKey(User, verbose_name=_("out user"), blank=True, null=True, on_delete=True)
    status = models.CharField(_("status"), max_length=const.DB_CHAR_CODE_2, default='0', choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"), blank=True, null=True)
...
class InOutDetail(models.Model):
    ...
    warehouse = models.ForeignKey(Warehouse, verbose_name=_("warehouse"), blank=True, null=True, on_delete=True)
    ...
    measure = models.ForeignKey(Measure, verbose_name=_("measure"), blank=True, null=True, on_delete=True)
...
class InitItem(InOutDetail):
    ...
    期初入库明细
    """
    master = models.ForeignKey(InitialInventory, on_delete=True)
...
class InItem(InOutDetail):
    ...
    入库单明细
    """
    master = models.ForeignKey(StockIn, on_delete=True)
    po_item = models.ForeignKey(POItem, verbose_name=_("po item"), blank=True, null=True, on_delete=True)
...
class OutItem(InOutDetail):
    ...
    出库单明细
    """
    master = models.ForeignKey(StockOut, on_delete=True)
    inventory = models.ForeignKey(Inventory, blank=True, null=True, verbose_name=_("inventory material"), on_delete=True)
...
class ReturnItem(InOutDetail):
    ...
    返库单明细
    """
    master = models.ForeignKey(WareReturn, on_delete=True)
    out_item = models.ForeignKey(OutItem, blank=True, null=True, verbose_name=_('out item'), on_delete=True)
...
class AdjustItem(InOutDetail):
    ...
    库存调整明细
    """
    master = models.ForeignKey(WareAdjust, on_delete=True)
    inventory = models.ForeignKey(Inventory, blank=True, null=True, verbose_name=_("inventory material"), on_delete=True)
